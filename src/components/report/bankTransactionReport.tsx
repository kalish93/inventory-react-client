...
function BankTransactionReportGenerator() {
  ...
  const handleGenerateReport = () => {
    dispatch(generateBankTransactionReport(selectedBank, startDate === '' ? undefined : new Date(startDate), endDate === '' ? undefined : new Date(endDate)));
    setStartDate('');
    setEndDate('');
  };
  ...
  const handleDownloadReport = () => {
    if (bankTransactionReport) {
      const blob = new Blob([bankTransactionReport], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
  
      // Concatenate bank name with the file name
      const fileName = `${selectedBank ? banks.items.find(bank => bank.id === selectedBank)?.name : 'bank'}-transaction-summary-${new Date().toLocaleDateString()}.pdf`;
