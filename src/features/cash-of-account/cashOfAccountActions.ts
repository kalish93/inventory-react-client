...
import { getCashOfAccountsFailure, getCashOfAccountsStart, getCashOfAccountsSuccess, registerCashOfAccountFailure, registerCashOfAccountStart, registerCashOfAccountSuccess } from "./cashOfAccountSlice";

export const getCashOfAccounts = (page?: number, pageSize?: number) => async (dispatch: AppDispatch) => {
    try {
  ...
  export const createCashOfAccount = (data: any) => async (dispatch: AppDispatch) => {
    try {
      ...
      if (response.success) {
        ...
        dispatch(registerCashOfAccountFailure(response.error || 'Unknown error'));
      }
    } catch (error) {
